...
func NewMVStates(txCount int, gasFeeReceivers []common.Address) *MVStates {
	m := &MVStates{
		...
		txDepCache:          make([]TxDep, 0, txCount),
...
func (s *MVStates) resolveDepsMapCacheByWrites(index int, rwSet *RWSet) {
	...
	if rwSet.excludedTx {
		s.txDepCache = append(s.txDepCache, NewTxDep([]uint64{}, ExcludedTxFlag))
		return
	}
	...
	for _, prev := range preDeps {
		for _, tx := range s.txDepCache[prev].TxIndexes {
			depMap.remove(tx)
		}
	}
	s.txDepCache = append(s.txDepCache, NewTxDep(depMap.deps()))
}
